...
<h1>版本说明</h1>
1.QQ对接chatgpt.py：最初的版本，进支持单个聊天，适用于图片等只需要单个的<br>
2.QQ对接chatgpt-1.1.py：第二次更新版本，支持连续对话，对话数据在交换成功后写入本地文件中，文件的生成是根据本地年月日+qq号生成的文件，存在当天会话数过多的可能且存在今天跟明天没有进行连续的问题。<br>
3.openai_config.json：文件新增几个参数给1.1版本支持。同时，考虑到有些代理跟项目中请求的地址是不一致的，因此在1.1的配置文件中需要自行添加地址。<br>
